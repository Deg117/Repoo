...
void generar_arch(Ts_alum vector[], int m);
int leer_arch(Ts_alum vector[], int m);
void eliminados(Ts_alum vector[], int m);
void contar_regis(Ts_alum vector[], int m);
...
void opciones()
{
    ...
    do
    {
        ...
        case 8:
            if (arch)
            {
                contar_regis(vector_reg, j);
            }
            else
            {
                printf("Genera un archivo si no que cuento\n"); // recordatorio de que esta vacio
            }
            break;
        case 9:
            if (elim)
...
void generar_arch(Ts_alum vector[], int m)
{
    ...
    fprintf(fa, "\n|   STATUS|  MATRICULA|   NOMBRE      |   APPAT       |   APMAT       |    EDAD|   SEXO  |\n");
    for (int i = 0; i < m; i++)
    {
        if (vector[i].status == 1)
        {
            fprintf(fa, "|   %6d|   %8d|   %-12s|   %-12s|   %-12s|   %5d|   %3d|\n",
                    vector[i].status, vector[i].matri, vector[i].nombre,
                    vector[i].apperno, vector[i].apmaterno, vector[i].edad, vector[i].sexo);
        }
    }
    printf("\n");
    //--------------------------------------------------------------------------------
...
int leer_arch(Ts_alum vector[], int m)
{
        ...
        {
            i = m;
            while (fscanf(fa, "%d %d %s %s %s %d %d",
                          &vector[i].status,
                          &vector[i].matri,
                          vector[i].nombre,
                          vector[i].apperno,// no ocupan & pq ya son una direccion 
                          vector[i].apmaterno,
                          &vector[i].edad,
                          &vector[i].sexo) == 7)
            {
...
void contar_regis(Ts_alum vector[], int m)
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        { // Solo contar los activos
            ...
            printf("|  %6d | %9d | %-12s | %-10s | %-10s | %4d | %-6s |\n",
                   vector[i].status, vector[i].matri, vector[i].nombre,
                   vector[i].apperno, vector[i].apmaterno, vector[i].edad, sexo);
        }
    }
    printf("\nTotal de registros activos: %d\n", activo);
}
